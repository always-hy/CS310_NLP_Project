...
# Load dataset
try:
    dataset = load_dataset('json', data_files="../dataset/english_ood/english_poem.json")
    df = dataset['train'].to_pandas()
except Exception as e:
    print(f"Error loading dataset: {e}")
    df = pd.read_json('../dataset/ood/english_poem.json')  # Fallback for single JSON array
...
# Evaluate model
def evaluate_model(model, test_loader, save_logits_path='../results/bert_english_logits_ood.pt'):
    all_preds = []
    all_labels = []
    all_probs = []
    all_logits = []
...
# Visualize metrics
{
    ...
    "options": {
        ...
        "plugins": {
            ...
            "title": {
                "display": true,
                "text": "Out-of-Domain Poetry Test Metrics"
            }
        }
    }
}